defmodule HelloLiveviewWeb.UserAuthTest do
  use HelloLiveviewWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias HelloLiveview.Accounts
  alias HelloLiveview.Accounts.Scope
  alias HelloLiveviewWeb.UserAuth

  import HelloLiveview.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, HelloLiveviewWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      HelloLiveviewWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        HelloLiveviewWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
